...
TYPE_MAPPER = {
    "alarm_control_panel": ("SecuritySystem", None),
    "binary_sensor": ("BinarySensor", None),
    "device_tracker": ("BinarySensor", None),
    "person": ("BinarySensor", None),
    "climate": ("Thermostat", None),
    "fan": ("Fan", None),
    "humidifier": ("HumidifierDehumidifier", None),
    "light": ("Light", None),
    "lock": ("Lock", None),
...
def get_accessory(
    ...
    a_type = get_type(state, config)
    
    if a_type is None:
        return None

    _LOGGER.debug('Add "%s" as "%s"', state.entity_id, a_type)
    return TYPES[a_type](hass, driver, config.get(CONF_NAME, state.name), state.entity_id, aid, config)

def get_type(state: State, config: dict) -> str | None:
    a_type, func = TYPE_MAPPER.get(state.domain, (None, None))

    if func is not None:
        a_type = func(state, config)
